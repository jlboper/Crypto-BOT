...
test('real workerd + local D1: browser session -> command -> Windows sync -> ack', {timeout:60000}, async t=>{
  ...
  const sql=['0001_portal.sql','0002_jobs.sql','0003_owner_password.sql','0004_bot_releases.sql','0005_restore_jobs.sql'].map(name=>readFileSync(new URL('../migrations/'+name,import.meta.url),'utf8')).join('\n');
