...
export async function checkMigrations({accountId,databaseId,token,expected,allowedPending=[]},transport=fetch){
  ...
  if(pending.some(name=>!allowedPending.includes(name)))throw Error('Pending D1 migration; review and apply it separately before publication');
