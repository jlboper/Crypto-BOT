...
export async function checkMigrations({accountId,databaseId,token,expected},transport=fetch){
  ...
  if(expected.some(name=>!applied.includes(name)))throw Error('Pending D1 migration; review and apply it separately before publication');
