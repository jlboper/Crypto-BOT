...
class AIAdvisor:
    ...
    def review_futures(self, signal: dict, account_context: dict[str, float]) -> AIReview:
        """Final fail-closed confirmation for a deterministic Futures Demo entry.

        Uses the same configured OpenAI model as Spot. It may only ALLOW or
        REJECT the already-proposed 1x BTCUSDT entry; it cannot create trades,
        change direction, increase size/leverage, or remove protections.
        """
        if not self.available:
            if self.settings.fail_closed:
                return AIReview("REJECT", 1.0, 0.0, "AI unavailable; fail-closed policy")
        ...
        payload = {
            ...
            "instructions": (
                ...
                "increase leverage or size, remove stop/take-profit protection, or infer missing market data. "
                "Reject inconsistent, overextended, ambiguous, or weakly supported setups. This is a demo environment."
            ),
