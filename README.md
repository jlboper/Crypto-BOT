...
La app de Windows dispone de un centro de actualización **local** y, desde 0.8.5, participa en un rollout firmado unificado. Tras aprobar/publicar una release, el portal remoto detecta una instalación Windows anterior y solicita automáticamente la instalación supervisada. El centro local sigue disponible para verificación, recuperación o instalación manual de un paquete ya descargado si el portal no está disponible. Solicita aprobación de la identificación exacta y usa el supervisor independiente para detener el motor activo de manera cooperativa, conservar respaldo y comprobar el arranque. El portal local mantiene la ruta web existente para quien prefiera ese flujo; ninguno instala ZIP sin firma. Para publicar una versión nueva, Work prepara un PR, GitHub prueba el commit y el propietario autoriza el entorno protegido desde el enlace mostrado. Esa aprobación cubre la publicación y el rollout automático del mismo paquete firmado a Windows; no existe una segunda aprobación rutinaria de instalación. Cloudflare recibe credenciales únicamente después de la aprobación; CI rechaza migraciones pendientes y hace rollback si la salud falla. Consulta `GITHUB_RELEASES.md`.
