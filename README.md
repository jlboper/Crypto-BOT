...
Desde 0.6.18, la app local sincroniza automáticamente los módulos del agente independiente desde la instalación firmada al iniciarse, después de un update local y tras detectar una instalación efectuada por el portal remoto mientras la app está abierta. Solo reinicia la tarea del agente cuando hay archivos nuevos verificados; conserva un respaldo y deja en el menú **Reparar conexión del portal** para incidencias. Si la versión anterior de la app ya estaba abierta antes de instalar 0.6.18, ciérrala y ábrela una vez para activar esta mejora. En 0.6.17 se retiraron las dos comprobaciones antiguas del indicador: el portal local/remoto permite elegir el modelo, reiniciar el motor PAPER y ver el estado de Testnet. `OPENAI_MODEL` presente en `.env.local` prevalece sobre `config.toml`; el control actualiza únicamente esa línea y comprueba el nuevo arranque.
